...
class StageNet(nn.Module):
    ...
    def __init__(self, args, ndepth, stage_idx):
        ...
        self.cost_reg_type = args.get(
            "cost_reg_type",
            ["Normal", "Normal", "Normal", "Normal"])[stage_idx]
        ...
        if self.fusion_type == 'cnn':
            self.vis = nn.Sequential(ConvBnReLU(1, 16), ConvBnReLU(16, 16),
                                     ConvBnReLU(16, 8), nn.Conv2d(8, 1, 1),
                                     nn.Sigmoid())
        else:
            raise NotImplementedError(
                f"Not implemented fusion type: {self.fusion_type}.")
        ...
        if self.cost_reg_type == "PureTransformerCostReg":
            ...
            self.cost_reg = PureTransformerCostReg(
                in_channels, **args['transformer_config'][stage_idx])
        else:
    ...
    def forward(self,
        ...
        assert len(src_feats) == len(
            proj_matrices
        ) - 1, "Different number of images and projection matrices"
        ...
        with torch.amp.autocast('cuda', enabled=False):
            for src_feat, src_proj in zip(src_feats, src_projs):
                ...
                if G < C:
                    ...
                    ref_volume = ref_feat.view(B, G, C // G, 1, H,
                                               W).repeat(1, 1, 1, D, 1,
                                                         1).to(torch.float32)
                    in_prod_vol = (ref_volume * warped_volume).mean(
                        dim=2)  # [B,G,D,H,W]
                elif G == C:
                ...
                if self.fusion_type == 'cnn':
                    ...
                    entropy = (-sim_vol_norm *
                               torch.log(sim_vol_norm + 1e-7)).sum(
                                   dim=1, keepdim=True)
        ...
        if self.depth_type == 'ce':
            if self.training:
                ...
                # vanilla argmax
                depth = torch.gather(depth_values,
                                     dim=1,
                                     index=idx.unsqueeze(1))[:, 0, ...]
            else:
                # regression (t)
                prob_volume_float = F.softmax(prob_volume_pre * tmp,
                                              dim=1).float()
                depth = depth_regression(prob_volume_float,
