...
class MemEffAttention(Attention):
    ...
    def forward(self,
        ...
        x = memory_efficient_attention(q,
                                       k,
                                       v,
                                       attn_bias=attn_bias,
                                       scale=softmax_scale)
        ...
        if return_attn:
            ...
            a = q[:, :, 0:1] @ k[:, :, 1:].transpose(
                -2,
                -1) * self.scale if softmax_scale is None else softmax_scale
...
class CrossLinearAttention(CrossAttention):
    ...
    def forward(self, x: Tensor, key=None, value=None, **kwargs):
        ...
        q = self.q_proj(x).reshape(B, N, self.num_heads,
                                   C // self.num_heads).to(dtype=torch.float32)
        k = self.k_proj(key).reshape(B, N, self.num_heads, C //
                                     self.num_heads).to(dtype=torch.float32)
        v = self.v_proj(value).reshape(B, N, self.num_heads, C //
                                       self.num_heads).to(dtype=torch.float32)
...
class CrossXFormersAttention(Attention):
    ...
    def forward(self,
        ...
        if attn_bias is not None:
            attn_bias = attn_bias.unsqueeze(1).repeat(
                1, self.num_heads, 1, 1).to(q.dtype)  # [B,HW,HW]->[B,nh,HW,HW]
        x = memory_efficient_attention(q,
                                       k,
                                       v,
                                       attn_bias=attn_bias,
